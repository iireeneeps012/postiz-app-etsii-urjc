...
export class UsersController {
  ...
  async getSelf(
    ...
    const org = organization as Organization & {
      subscription?: {
        ...
        subscriptionTier?: 'FREE' | 'STANDARD' | 'PRO' | 'ULTIMATE' | 'TEAM';
        isLifetime?: boolean;
      };
      users?: Array<{
  ...
  async changePassword(
    ...
    const loadedUser = await this._userService.getUserById(user.id);
    if (
      !loadedUser ||
      loadedUser.providerName !== 'LOCAL' ||
      !loadedUser.password
    ) {
      throw new HttpException('Password change is not available', 400);
    }

    if (
      !AuthChecker.comparePassword(body.currentPassword, loadedUser.password)
    ) {
