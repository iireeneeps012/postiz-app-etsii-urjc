...
export class PermissionsService {
  ...
  async check(
    ...
    const isAdmin = ['ADMIN', 'SUPERADMIN'].includes(permission);

    for (const [action, section] of requestedPermission) {
      if (section === Sections.ADMIN && isAdmin) {
        can(action, section);
      }

      if (section === Sections.TEAM_MEMBERS && isAdmin) {
        can(action, section);
      }

      if (section === Sections.CHANNEL && isAdmin) {
        can(action, section);
      }

      if (section === Sections.WEBHOOKS && isAdmin) {
        can(action, section);
      }
    }
