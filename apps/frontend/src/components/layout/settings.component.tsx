...
export const SettingsPopup: FC<{
  ...
  const list = useMemo(() => {
    ...
    return arr;
  }, [user, isGeneral, showLogout, t, isAdmin]);
  ...
  return (
    <>
      ...
      <div className="bg-newBgColorInner flex-1 flex-col flex p-[20px] gap-[12px]">
        <div
          className={clsx(
            'w-full mx-auto gap-[24px] flex flex-col relative rounded-[4px]'
          )}
        >
          {tab === 'global_settings' && (
            <div>
          ...
          {tab === 'webhooks' && isAdmin && !!user?.tier?.webhooks && (
            <div>
              <Webhooks />
            </div>
          )}
          ...
          {tab === 'sets' && isAdmin && user?.tier.current !== 'FREE' && (
            <div>
              <Sets />
            </div>
          )}

          {tab === 'signatures' && isAdmin && user?.tier.current !== 'FREE' && (
            <div>
              <SignaturesComponent />
            </div>
          )}
