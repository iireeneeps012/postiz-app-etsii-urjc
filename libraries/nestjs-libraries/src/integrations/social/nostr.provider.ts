...
const getPool = async () => {
  if (!poolInstance) {
    const { SimplePool } = await (eval('import("nostr-tools")'));
...
export class NostrProvider extends SocialAbstract implements SocialProvider {
  ...
  private async publish(pubkey: string, event: any) {
    const { Relay } = await (eval('import("nostr-tools")'));
    let id = '';
  ...
  async authenticate(params: {
    ...
    try {
      const { getPublicKey } = await (eval('import("nostr-tools")'));
  ...
  async post(
    ...
    const { finalizeEvent } = await (eval('import("nostr-tools")'));
